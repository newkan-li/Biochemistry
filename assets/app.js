...
(function () {
  ...
  function exportAllPDF() {
    ...
    var P = ['<h1>生物化学 · 全部章节（题目与我的作答）</h1>', '<p class="meta">导出时间：' + esc(new Date().toLocaleString("zh-CN")) + '</p>'];
    M.forEach(function (m) { var ch = window.CHAPTERS[m.id]; if (ch) P.push(buildChapterDoc(m.id, ch)); });
    printDoc(P.join(""), "生物化学 全部题目与作答");
  }
  ...
  function exportAllCardsPDF() {
    var M = window.MANIFEST || [];
    var P = ['<h1>生物化学 · 全部闪卡</h1>', '<p class="meta">导出时间：' + esc(new Date().toLocaleString("zh-CN")) + '</p>'];
    M.forEach(function (m) { var ch = window.CHAPTERS[m.id]; if (ch) P.push(buildCardsDoc(m.id, ch)); });
    printDoc(P.join(""), "生物化学 全部闪卡", CARDS_CSS);
  }
  ...
  function maybeRemind() {
    ...
    if (hm >= rm.time && rm.last !== today) {
      ...
      try { new Notification("生物化学 · 今日复习", { body: "该复习啦：打开网站完成今日任务。" }); } catch (e) { }
    }
  }
  ...
  function backupAll() {
    ...
    var a = el("a"); a.href = URL.createObjectURL(blob); a.download = "biochem-backup-all.json"; a.click();
  }
